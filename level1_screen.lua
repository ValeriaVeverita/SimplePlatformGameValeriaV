...
local function onCollision( self, event )
    ...
    if ( event.phase == "began" ) then
        ...
        if  (event.target.myName == "spikes1") or 
            ...
            spikeChannel = audio.play(spikeSound)
...
function scene:create( event )
    ...
    --SOUND--
    winSound = audio.loadStream("Sounds/YouWin.wav")
    loseSound = audio.loadStream("Sounds/YouLose.mp3")
    spikeSound = audio.loadStream("Sounds/Pop.mp3")
    ...
    rightW = display.newRect( 5, 300, 5, 1500)
    rightW:setFillColor(0,0,0)
